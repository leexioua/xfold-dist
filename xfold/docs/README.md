...
## 1. 软件背景
- 日常使用电脑，经常需要进行一连串的按键和鼠标操作，才能完成一项任务。比如，搜索资料，首先打开浏览器，输入打开搜索引擎首页，然后输入关键词后，点击完成搜索，过程十分繁琐。本软件旨在通过定义命令行或快捷键，简单快速完成这些操作过程，实现时间节省（折叠时间）。
- 常用场景：网页搜索，查询英语单词，查找本地文件，打开常用文件，保存摘抄笔记等等
- 软件名由来： 折叠时间 -> fold，提升效率 -> \*10 <==> xfold · ten-fold（十倍效率）
- 本软件初衷为作者自用，所以命令行及快捷键的定义，均使用文本编辑器修改扩展功能，高级用户可参考样例进行扩展更多功能，目前暂无开发配置界面计划。

## 2. 功能简介
```js
xfold 是一个效率提升工具，支持将一连串的操作配置定义成命令行或快捷键，方便后续使用，开箱默认提供一些常用功能项。
1. 软件开箱默认提供实用命令及快捷键功能，例如，划词查单词，网页搜索，快速打开，笔记收藏等。
2. 扩展支持LButton组合键，双键，三键以及长按作为快捷键，缓解快捷键占用，不易记忆问题。
3. 内置基础操作功能，如打开文件、目录、网址和打开应用程序窗口，发送按键，交互输入等，通过组合可定义出更多功能。
4. 参考示例修改，可以定义扩展更多的功能，指定快捷键或命令行，方便后续使用。
```

### 2.1 特色功能说明
- 扩展LButton组合快捷键，更友好，告别热键冲突
	+ 如：LButton+g，指按住鼠标左键先不松开，然后敲击键 <kbd>g</kbd>
	+ 适用于鼠标划词或选择文本或文件时操作的场景，例如谷歌搜索选择的文本
	+ <kbd>ESC</kbd> 或 <kbd>\\</kbd> 键可作为LButton替代按键，LButton+e 等同 ESC+e 等同 \+e 
		```js
		注：当操作的窗口按Esc触发关闭或取消选择时，建议使用LButton+e或\+e
		```
- 扩展双键，三键，长按快捷键，扩充出更多快捷键
	+ Ctrl/Alt/Shift等组合快捷键在很多应用中被占用
	+ 提供更多快捷键供选择，同时有助快捷键记忆
	+ 使用简单顺手，如：双击F1，相比Ctrl+F1更顺手
- 支持只在指定应用内生效的快捷键
	```js
...
- 支持热字串功能，支持扩展定义 
	...
	- ..r： 输入框输入 "..r" -> xfold运行输入框内的文本
	- ..t： 输入框输入 "..t" -> 输出xfold运行输入框内的文本返回的字符串
	- ..v： 输入框输入 "..v" -> 输出xfold获取输入框内文本的变量值
	- ..z： 输入框输入 "..z" -> 弹出xfold命令行交互窗口
